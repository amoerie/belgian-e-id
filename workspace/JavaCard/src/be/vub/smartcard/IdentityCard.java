...
public class IdentityCard extends Applet {
	...
	private void getSerial(APDU apdu){
		...
		if(!pin.isValidated())ISOException.throwIt(SW_PIN_VERIFICATION_REQUIRED);
			...
			apdu.setOutgoing(); // you can not allocate memory, you must used your input, it clears your APDU
			apdu.setOutgoingLength((short)serial.length); // 0 if you don't want to send something, the lenght of the data
			apdu.sendBytesLong(
					serial, //array that you want to send
					(short)0, //offset
					(short)serial.length);  //if you want to send more data!! you must send the data in chunks! If error <-- smaller chunks
		}
	}
}
